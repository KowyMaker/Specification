...
public class CatmullRom implements TweenPath
{
    ...
    public float compute(float t, float[] points, int pointsCnt)
    {
        ...
        if (segment == 0)
        {
            return catmullRomSpline(points[0], points[0], points[1], points[2],
                    t);
        }
        
        if (segment == pointsCnt - 2)
        {
            return catmullRomSpline(points[pointsCnt - 3],
                    points[pointsCnt - 2], points[pointsCnt - 1],
                    points[pointsCnt - 1], t);
        }
        
        return catmullRomSpline(points[segment - 1], points[segment],
    ...
    private float catmullRomSpline(float a, float b, float c, float d, float t)
    {
        ...
        final float h1 = +2 * t * t * t - 3 * t * t + 1;
        final float h2 = -2 * t * t * t + 3 * t * t;
        final float h3 = t * t * t - 2 * t * t + t;
