...
public abstract class BaseTween<T>
{
    ...
    private boolean       isStarted;          // true when the object is
                                               // started
    private boolean       isInitialized;      // true after the delay
    private boolean       isFinished;         // true when all repetitions are
                                               // done
    ...
    private void testRelaunch()
    {
                ...
                && currentTime + deltaTime >= 0)
        {
            ...
            isIterationStep = true;
            step = 0;
            final float delta = 0 - currentTime;
            deltaTime -= delta;
            currentTime = 0;
            callCallback(TweenCallback.BEGIN);
            callCallback(TweenCallback.START);
    ...
    private void updateStep()
    {
        while (isValid(step))
        {
            ...
            else if (!isIterationStep && currentTime + deltaTime >= repeatDelay)
            {
                ...
                final float delta = repeatDelay - currentTime;
                deltaTime -= delta;
                currentTime = 0;
            ...
            else if (isIterationStep && currentTime + deltaTime > duration)
            {
                ...
                final float delta = duration - currentTime;
                deltaTime -= delta;
                currentTime = duration;
