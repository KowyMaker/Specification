...
public final class Tween extends BaseTween<Tween>
{
    ...
    private static final Pool<Tween>          pool         = new Pool<Tween>(
                                                                   20,
                                                                   poolCallback) {
                                                               @Override
     ...
     *            "someClass".
     */
    public static void registerAccessor(Class<?> someClass,
    ...
    private final float[]         waypoints      = new float[waypointsLimit
                                                         * combinedAttrsLimit];
    
    // Buffers
    private float[]               accessorBuffer = new float[combinedAttrsLimit];
    private float[]               pathBuffer     = new float[(2 + waypointsLimit)
                                                         * combinedAttrsLimit];
    ...
    private Class<?> findTargetClass()
    {
        ...
        while (parentClass != null
                && !registeredAccessors.containsKey(parentClass))
        {
    ...
    public Tween waypoint(float... targetValues)
    {
        ...
        System.arraycopy(targetValues, 0, waypoints, waypointsCnt
    ...
    protected void initializeOverride()
    {
        ...
        for (int i = 0; i < combinedAttrsCnt; i++)
        {
            ...
            {
                waypoints[ii * combinedAttrsCnt + i] += isRelative ? startValues[i]
                        : 0;
            }
            ...
            boolean isIterationStep, float delta)
    {
        ...
        {
            accessor.setValues(target, type, isReverse(lastStep) ? startValues
                    : targetValues);
            return;
        }
        ...
        if (!isIterationStep && step < lastStep)
        {
            accessor.setValues(target, type, isReverse(lastStep) ? targetValues
                    : startValues);
            return;
        }
        ...
        {
            accessor.setValues(target, type, isReverse(step) ? targetValues
                    : startValues);
            return;
        }
        ...
        if (duration < 0.00000000001f && delta < 0.00000000001f)
        {
            accessor.setValues(target, type, isReverse(step) ? startValues
                    : targetValues);
            return;
        }
        ...
        final float time = isReverse(step) ? duration - getCurrentTime()
                : getCurrentTime();
        final float t = equation.compute(time / duration);
    ...
    {
        final String msg = "You cannot combine more than "
                + combinedAttrsLimit
                + " "
    ...
    {
        final String msg = "You cannot add more than " + waypointsLimit + " "
